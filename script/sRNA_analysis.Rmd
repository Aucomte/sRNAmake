...
# rootOutputPath <- "/data3/projects/pixies/sRNA/PXO99_Meyer/RUN_20190219" # "/data3/projects/pixies/TMP"
# genomicDir <- "/data3/projects/pixies/reference_genomic_data/NB_genome"
# scriptsDir <- "/data3/projects/pixies/scripts"
# outDirSStack <- file.path(rootOutputPath, "shortStack")
# outDirBWA <- file.path(rootOutputPath, "mappedBWA")
# # TEMP PARAMETER PATHS FOR TESTING
# ncores <- 3
# sampleInfoBamFiles <- file.path(outDirBWA, "sampleInfo.txt")
# outDirOnNode <- file.path(rootOutputPath, "sRNA_analysis")
# dir.exists(outDirOnNode) || dir.create(path = outDirOnNode, mode = "770")
# genomeGffFile <- file.path(genomicDir, "MSU7.gff3")
# genomeFastaFile <- file.path(genomicDir, "NUC_MSU7.fa")
# miRnaGffFile <- file.path(genomicDir, "miRBase_MSU7_MIR.gff3")
# sstackGffFile <- file.path(outDirSStack, "ShortStack_All.gff3")
# comparisonsFile <- file.path(scriptsDir, "treatmentsComparisons_Meyer.csv")
# xisrnaBai3File <- file.path(scriptsDir, "xisRNA_BAI3.csv")
###################################################
# Testing with degradome on genome dataset
# rootOutputPath <- "/data3/projects/pixies/TMP/degradome"
# genomicDir <- "/data3/projects/pixies/reference_genomic_data/NB_genome"
# scriptsDir <- "/data3/projects/pixies/scripts"
# outDirSStack <- file.path(rootOutputPath, "shortStack")
# outDirBWA <- file.path(rootOutputPath, "mapped")
# # TEMP PARAMETER PATHS FOR TESTING
# ncores <- 6
# sampleInfoBamFiles <- file.path(outDirBWA, "sampleInfo.txt")
# outDirOnNode <- file.path(rootOutputPath, "sRNA_analysis")
...
# miRnaGffFile <- file.path(genomicDir, "miRBase_MSU7_MIR.gff3")
# sstackGffFile <- file.path(outDirSStack, "ShortStack_All.gff3")
# comparisonsFile <- file.path(scriptsDir, "treatmentsComparisons_AcobiomDegradome2018.csv")
# xisrnaBai3File <- file.path(scriptsDir, "xisRNA_BAI3.csv")
###################################################

# Getting parameters values from calling rmarkdown::render fonction.
ncores <- as.integer(params$ncores)
sampleInfoBamFiles <- params$sampleInfoBamFiles 
outDirOnNode <- params$outDirOnNode
invisible(dir.exists(outDirOnNode) || dir.create(path = outDirOnNode, mode = "770"))
genomeGffFile <- params$genomeGffFile
genomeFastaFile <- params$genomeFastaFile
miRnaGffFile <- params$miRnaGffFile
sstackGffFile <- params$sstackGffFile
comparisonsFile <- params$comparisonsFile
xisrnaBai3File <- params$xisrnaBai3File
scriptsDir <- params$scriptsDir


#sampleInfoFastqFiles
#bamMergedFiles
#sampleInfoMergedBamFiles
...
```{r loadPackages}
suppressPackageStartupMessages(library("parallel")) # For parrallel computin
...
suppressPackageStartupMessages(library("segmentSeq"))
...
source(file = file.path(scriptsDir, "sRNA_analysis_lib.R"))
```
...
## Read paths and exerimental factors from sampleInfo file
...
## Reads and mapping files containers with assoctiated experimental metadata
...
# More path locations
plotsByClusterDir <- file.path(outDirOnNode, "PlotsByCluster")
...
alpha <- 0.05
maxAdjPval <- 0.05 # Results filtering criteria
...
Right now the code is a bit clumsy...

```{r locationFiltering}
# clumsily getting annotation from the NCBI annotation of the IRGSP genome assembly  with 'proper' seqlevels
NipNCBIAnnot <- rtracklayer::import(file.path(dirname(genomeGffFile), "Oryza_sativa_genomic_refseq_IRGSP_seqlevels.gff3"), format = "gff3")

# Find in this annotation GRanges corresponding to tRNA, rRNA et al.
unwantedTypes <- c("tRNA", "rRNA", "pseudogenic_tRNA")
toBeFilteredOutGR <- NipNCBIAnnot[NipNCBIAnnot$type %in% unwantedTypes, ]


# Filtering out features that overlap with annotation elements that should not be counted.
featuresOutsideUnwanted <- sstackClustCounts  %outside% toBeFilteredOutGR
locationFilteringStats <- as.data.frame(table(featuresOutsideUnwanted))
sstackClustCounts <- sstackClustCounts[featuresOutsideUnwanted, ]
...
write.csv(file = "sstackClustCounts.csv", cts)
save(file = "sstackClustCounts.Rdata", sstackClustCounts)
```
...
do.call(pairwiseScatterPlots, SARToolsParamLst[-3])
```
...
for (i in 1:length(deseq2Output$results)) {
  ...
  summary(deseq2Output$results[[i]])
  }
```
...
```{r dispEstimPlot, fig.width=8, eval= replicated}
...
loci_annot <- HeatmapAnnotation(MCluster = annotFactor, show_annotation_name = TRUE,
                                col = list(MCluster = colorsForFactor(annotFactor)), which = "row")
...
cols = lapply(annotDf, colorsForFactor)
...
## Overlap with xisRNA across comparisons

**May want to omit this step**

### Table summarizing the overlap between the list xisRNA and the diff accuml clusters:
```{r}
DT::datatable(
  DELociExprStats[
  !is.na(DELociExprStats[ , "OlapXisRNA_Names"]),
  c("OlapGene_Names","OlapSimpleTrx_Names" , "OlapGene_Note", "comparison",  "OlapXisRNA_Names")
                        ]
)
```
...
Display a heatmap with xisRNA clusters as rows and treatment comparisons as column and indicating with 1 if the xisRNA appears in the diffAccumul clusters of treatment comparison:


```{r, eval = all(countOfSets > 1, na.rm = TRUE), fig.width=9, fig.height=9}
mat <- unclass(xtabs(formula =  ~ OlapXisRNA_Names + comparison, data = DELociExprStats)) # Have to clear xtabs class otherwise Heatmap complains...
...
if (nrow(mat) != 0) {
  ...
  # Create a heatmap of the lesion length data with the dendograms
  xisByComp_HM <- Heatmap(mat,
                          name = "HClustering of xisRNA loci found in the set of treatment comparisons",
                          heatmap_legend_param = list(title = "Detected as differentially expressed"),
                          col = c("azure", "darkorange"),
                          column_title = "Treatment comparisons", column_title_side = "top",
                          column_title_gp = gpar(lineheight = 0.9),
...
idsToBePloted <- as.matrix(mcols(myDEloci)[!is.na(mcols(myDEloci)[ , "OlapXisRNA_Names"]), c("id", "OlapXisRNA_Names")])
...
write.table(DELociExprStats,
		file = paste0("sRNALoci_DE_stats_", chartr(" ", "_", chartr(":", "-", trunc(Sys.time(), "secs"))), ".txt"),
...
write.table(AllLociExprStats,
		file = paste0("sRNAloci_ALL_stats_", chartr(" ", "_", chartr(":", "-", trunc(Sys.time(), "secs"))), ".txt"),
		append = FALSE, quote = FALSE, sep = "\t",
		eol = "\n", na = "", dec = ".", row.names = FALSE,
		col.names = TRUE)
...
write.table(as.data.frame(mcols(myDEloci), optional = TRUE),
		file = paste0("sRNALoci_features_", chartr(" ", "_", chartr(":", "-", trunc(Sys.time(), "secs"))), ".txt"),
		append = FALSE, quote = FALSE, sep = "\t",
		eol = "\n", na = "", dec = ".", row.names = FALSE,
		col.names = TRUE)

save(file = "myDEloci.Rdata", myDEloci)

### Use rtracklayer to export a gff3 file with all the info
rtracklayer::export(myDEloci,"myDEloci.gff3", "gff3")
...
```{r}
save.image(file = "sRNA_analysis_workspace.RData")
```
