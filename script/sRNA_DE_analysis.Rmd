...
# # #############   INITIAL TESTING  ################
# sampleInfoBamFiles <- "/data/projects/rymv_xantho_smallrna/seb/bai3_dataset_output/2_mapping_sRNA/bamfile_info.txt"
# genomeGffFile <- "/data/projects/rymv_xantho_smallrna/seb/bai3_dataset_output/1_cat_gtf/all_gtf.gtf"
# genomeFastaFile <- "/data/projects/rymv_xantho_smallrna/seb/bai3_dataset_output/1_cat_ref/all_ref.fa"
# sstackGffFile <- "/data/projects/rymv_xantho_smallrna/seb/bai3_dataset_output/4_ShortStack/ShortStack_All_miRNA.gff3"
# ncores <- 6 # snakemake@threads
# outDir <- "/data/projects/rymv_xantho_smallrna/seb/bai3_dataset_output/5_sRNA_loci_DE_analysis"
# invisible(dir.exists(outDir) || dir.create(path = outDir, mode = "770"))
# comparisonsFile <- "/data3/projects/pixies/scripts/treatmentsComparisons_2012.csv"
# filterGffFile <- "/data3/projects/pixies/reference_genomic_data/NB_genome/Oryza_sativa_genomic_refseq_IRGSP_seqlevels.gff3"
...
#sampleInfoBamFiles <- "/data/projects/rymv_xantho_smallrna/srnamake/output2rpm/2_mapping_sRNA/bamfile_info.txt"
#genomeGffFile <- "/data/projects/rymv_xantho_smallrna/srnamake/DATA2/annotation/RiceRYMV2.gtf"
#genomeFastaFile <- "/data/projects/rymv_xantho_smallrna/srnamake/output2rpm/1_cat_ref/all_ref.fa"
#sstackGffFile <- "/data/projects/rymv_xantho_smallrna/srnamake/output2rpm/4_ShortStack/ShortStack_All_miRNA.gff3"
#ncores <- 6 # snakemake@threads
#outDir <- "/data/projects/rymv_xantho_smallrna/srnamake/output2rpm/5_sRNA_loci_DE_analysis_test"
#invisible(dir.exists(outDir) || dir.create(path = outDir, mode = "770"))
#comparisonsFile <- "/data/projects/rymv_xantho_smallrna/srnamake/DATA2/treatmentsComparisons.csv"
#filterGffFile <- "/data/projects/rymv_xantho_smallrna/srnamake/DATA2/Oryza_sativa_genomic_refseq_IRGSP_seqlevels.gff3"
 # feature filtering based on total read counts
#minRowSumTreshold <- 30 # snakemake@params[["minRowSumTreshold"]]
 # DESeq2 fonctions parameters values
#variableOfInterest <- "Treatment"
...
## Printing parameter values provided for this run

```{r print snakemake object content}
str(snakemake)
```
...
```{r install and load R packages and functions}
...
userLibPath <- normalizePath(Sys.getenv("R_LIBS_USER"))
if(!file.exists(userLibPath)) {
  warning("Creating the R library installation directory:", userLibPath, "\n")
...
if (!"SARTools" %in% installed.packages(lib.loc = userLibPath)[, "Package"]) {
  warning("The SARTools package is unavailable and will be installed in:\n", userLibPath, "\n")
  devtools::install_github("PF2-pasteur-fr/SARTools", build_opts="--no-resave-data",
                           dependencies = TRUE, upgrade = FALSE,
                           lib = userLibPath
  )
}
myLoadInstall(necessaryPackages, lib = userLibPath)
...
if (!replicated) cat("No duplicate sample has been detected in the dataset!",
 "Analysis will proceed with specific parameters.",
  "Statistical inference on differential accumulation will be performed using the DESEq package and many of the QC steps will be omitted!\n")
```
...
### Reference genome annotation and sequence

```{r refGenomeAnnot}
refGenomeSeq <-FaFile(genomeFastaFile)
genomeFastaIndexFile <- indexFa(genomeFastaFile)
chrominfo <- Rsamtools::seqinfo(refGenomeSeq)
genomeAnnotDB <- makeTxDbFromGFF(genomeGffFile, chrominfo = chrominfo )
refGenes <- GenomicFeatures::genes(genomeAnnotDB)
...
**Loading experimental sRNA clusters from genome segmentation-based inference method output**
...
**!! THIS LINE BELOW CAN BREAK EVERYTHING IF GTF DOES NOT HAVE THE CORRESPONDING COLS!!!! **


```{r}
sRnaClusters <- addRefAnnot(querGR = sRnaClusters, refGR = refGenes,  mcolsToKeep = "gene_id", prefix = "OlapGene_")
```
...
`r knitr::kable(abundFilteringStats, format = "markdown")`
- Counts of features removed because they overlapped with tRNA, rRNA, ... elements
`r knitr::kable(locationFilteringStats, format = "markdown")`
...
df <- cbind(
as.data.frame(mcols(sRnaClusters[row.names(majFeatures)])),
majFeatures)
DT::datatable(df)
```
...
```{r eval= replicated}
deseq2Output <- runDESeq2(se = sstackClustCounts,
                          varInt = variableOfInterest,
                          batch = batch,
                          comps = comparisons, 
                          locfunc=locfunc,
                          fitType=fitType,
                          sizeFactEstimMethod = sizeFactEstimMethod,
...
```{r check DELociExprStats and Create a GRanges container with DE loci}

if(nrow(DELociExprStats) < 1) {
  cat(" ## It appears that the set of features detected as differentially expressed is empty!!\n",
      "## You may want to use a less stringent set of DE parameters.\n",
      "## Quitting...")
  sessionInfo()
...
```{r mClustDEloci, eval = all(replicated, na.rm = TRUE), fig.height=8, fig.width=8}
...
```{r DElociExpressionHeatMap, eval = all(replicated, na.rm = TRUE), fig.width= 12, fig.height= 14}
...
## Expression plot of DE sRNA loci across experimental treatments
